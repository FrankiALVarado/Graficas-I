...
CDevice								g_Device;
CDeviceContext                      g_DeviceContext;
//ID3D11Device*                       g_pd3dDevice = NULL;
//ID3D11DeviceContext*                g_pImmediateContext = NULL;
CSwapcChain							g_SwapChain;
//IDXGISwapChain*                     g_pSwapChain = NULL;
ID3D11RenderTargetView*             g_pRenderTargetView = NULL;

CDepthStencil						g_DepthStencil;
...
ID3D11Buffer*                       g_pCBNeverChanges = NULL; //buffer for updating the camera
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
	...
	g_Manager.InitImGui(g_Device.m_device, g_DeviceContext.m_DeviceContext, g_hWnd);
	g_Manager.loadScene();
    // Main message loop
    MSG msg = {0};
    while( WM_QUIT != msg.message )
    {
		...
		else if (WM_EXITSIZEMOVE == msg.message) {
			//actualizar texturas y todo jeje
		}
		else if (WM_KEYDOWN == msg.message) {
			switch (msg.wParam)
			{
			case VK_LEFT:
				g_View *= XMMatrixTranspose(XMMatrixTranslationFromVector(XMVECTOR{ 3.0, 0, 0 }));
				g_DeviceContext.updateSubResource(g_pCBNeverChanges, 0, NULL, &g_View, 0, 0);
				Render();
				break;
			case VK_RIGHT:
				g_View *= XMMatrixTranspose(XMMatrixTranslationFromVector(XMVECTOR{ -3.0, 0, 0 }));
...
HRESULT InitDevice()
{
    ...
    for( UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++ )
    {
        ...
        hr = D3D11CreateDeviceAndSwapChain( NULL, g_driverType, NULL, createDeviceFlags, featureLevels, numFeatureLevels,
                                            D3D11_SDK_VERSION, &sd, g_SwapChain.getSwapChain(), g_Device.getDevice(), &g_featureLevel, g_DeviceContext.getDeviceContext() );
        if( SUCCEEDED( hr ) )
            break;
    }
    if( FAILED( hr ) )
	...
	hr = g_Device.createTexture2D(&descDepth, NULL, g_DepthStencil.getDepthStencil());
    if( FAILED( hr ) )
        return hr;
   ...
   // hr = g_pd3dDevice->CreateDepthStencilView( g_pDepthStencil, &descDSV, &g_pDepthStencilView );
	hr = g_Device.createDepthStencilView(g_DepthStencil.getDepthStencilPointer(), &descDSV, &g_pDepthStencilView);
    if( FAILED( hr ) )
        return hr;
   ...
   // hr = g_pd3dDevice->CreateInputLayout( layout, numElements, pVSBlob->GetBufferPointer(),
    ...
    hr = CompileShaderFromFile( L"Tutorial07.fx", "CP", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
                    L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK );
   ...
   // hr = g_pd3dDevice->CreateBuffer( &bd, &InitData, &g_pIndexBuffer );
	...
	g_DeviceContext.IAsetIndexBuffers(g_pIndexBuffer, DXGI_FORMAT_R16_UINT, 0);
    ...
    // Load the Texture
    hr = D3DX11CreateShaderResourceViewFromFile( g_Device.getPointerDevice(), L"seafloor.dds", NULL, NULL, &g_pTextureRV, NULL );
    if( FAILED( hr ) )
        return hr;
...
void CleanupDevice()
{
	...
	;
	if (g_DeviceContext.getDevicePointer()) g_DeviceContext.clearState();
	...
	if (g_DepthStencil.getDepthStencilPointer()) g_DepthStencil.release();
    //if( g_pDepthStencil ) g_pDepthStencil->Release();
    if( g_pDepthStencilView ) g_pDepthStencilView->Release();
    if( g_pRenderTargetView ) g_pRenderTargetView->Release();
	if (g_SwapChain.getSwapChainPointer()) g_SwapChain.release();
    //if( g_pSwapChain ) g_pSwapChain->Release();
	//if( g_pImmediateContext ) g_pImmediateContext->Release();
	if (g_DeviceContext.getDevicePointer()) g_DeviceContext.release();
    //if( g_pd3dDevice ) g_pd3dDevice->Release();
	if (g_Device.getPointerDevice()) g_Device.release();
}
...
void Render()
{
	...
	cb.mWorld = XMMatrixTranspose(XMMatrixScaling(f, f, f));
	...
	cb.mWorld = XMMatrixTranspose(XMMatrixTranslationFromVector(XMVECTOR{ 2.8,0,0 }));
	cb.mWorld *= XMMatrixTranspose(XMMatrixScaling( f, f, f));
